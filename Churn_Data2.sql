...
((COUNT(gender))*100)/(SELECT COUNT(*) FROM Customer_Data ) AS gender_percent
FROM Customer_Data  
GROUP BY gender
...
(SUM(total_revenue)*100)/(SELECT SUM(total_revenue) FROM Customer_Data)  AS rev_percent
FROM Customer_Data
...
--Removing NULL and insert new data into prod table
SELECT 
    Customer_ID,
